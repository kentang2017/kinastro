...
![Systems](https://img.shields.io/badge/Systems-81-FFD700?style=for-the-badge)
...
| | 中文 | English |
|---|---|---|
| 🔮 **八十一體系合一** | 在同一個介面中切換八十一種占星體系，無需來回切換工具 | Switch between 81 astrology systems in one unified interface |
...
> 💡 體系按分類顯示：熱門 → 三式 → 中國 → 西方 → 印度 → 亞洲 → 中東 → 非洲 → 古代文明 → 隱祕與原民傳統 → 醫占 (含拜占庭占星) → 傳統卜卦占星 → 擇日占星；目前清單已與側邊欄同步，納入馬來伊斯蘭占星 Bintang Duabelas（十二星）、馬來七星五刻占卜 Kinketika、地占占星（Astronomical Geomancy）、伊特魯里亞占星（Etruscan Astrology）、吠陀風水（Vastu Shastra）、歐洲地占（European Geomancy）、弗拉德命運輪盤（Fludd Rota Simulator）、煉金占星（Alchemical Astrology）與衣索比亞 Bahre Hasab（Ethiopian Bahre Hasab）與亞美尼亞占星（Armenian Astrology）
>
> *Systems displayed by category: Popular → Three Formulae → Chinese → Western → Indian → Asian → Middle East → Africa → Ancient → Obscure & Indigenous → Medical Astrology (incl. Byzantine Astrology) → Traditional Horary → Electional; the list is now aligned with the sidebar and includes Bintang Duabelas (Twelve Stars), Kinketika (Malay Time Divination), Astronomical Geomancy, Etruscan Astrology, Vastu Shastra, European Geomancy, Fludd Rota Simulator, Alchemical Astrology, Ethiopian Bahre Hasab, and Armenian Astrology*
