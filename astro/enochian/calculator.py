...
def compute_enochian_chart(
    ...
    """
    # 1. 計算儒略日
    jd = _get_julian_day(year, month, day, float(hour), float(minute), float(timezone))

    # 2. 計算行星位置
    planet_positions = _compute_planet_positions(jd)

    # 3. 計算宮位
    house_cusps, asc_lon, mc_lon = _compute_houses(jd, float(latitude), float(longitude))
    ...
    for planet_name in _SWE_IDS:
        lon, is_retro = planet_positions[planet_name]
        sign, sign_zh, sign_degree = _get_sign(lon)
        house = _get_house_for_longitude(lon, house_cusps)
    ...
    for i, sign in enumerate(_SIGNS):
        if sign == sun_sign:
            sigillum_idx = i % 7
            break
    sigillum_angels = SIGILLUM_DEI_AEMETH["seven_angels"][sigillum_idx:sigillum_idx + 3]
    sigillum_personal_number = sigillum_idx + 1

    # 12. 整體解讀
    overall_en, overall_zh = _build_overall_path(dominant_watchtower, dominant_element, primary_aethyr, patron_angel)
    magical_en, magical_zh = _build_magical_purpose(planet_points, watchtower_scores)
