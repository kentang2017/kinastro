"""巴比倫 (Babylonian) Astrology handler."""
...
def build_babylonian_handler(
    *,
    compute_babylonian_chart,
    render_babylonian_chart,
    ai_button_sink,
) -> SystemHandler:
    """Create executable handler for 巴比倫 (Babylonian) Astrology."""
    ...
    def _cached_compute(params_payload: dict[str, Any], **extra_kwargs):
        ...
        return compute_babylonian_chart(**params_payload, **extra_kwargs)
    ...
    def _render(result: Any, params: BirthChartParams, options: dict[str, Any]) -> None:
        ...
        render_babylonian_chart(
            ...
            after_chart_hook=lambda: ai_button_sink(
                "tab_babylonian", result, "babylonian", ""
            ),
        )

    return SystemHandler(
        system_id="tab_babylonian",
        compute=_compute,
        render=_render,
