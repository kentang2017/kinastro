"""策天飛星 (Ce Tian) Astrology handler."""
...
def build_cetian_handler(
    *,
    compute_cetian_ziwei_chart,
    render_cetian_ziwei_chart,
    ai_button_sink,
) -> SystemHandler:
    """Create executable handler for 策天飛星 (Ce Tian) Astrology."""
    ...
    def _cached_compute(params_payload: dict[str, Any], **extra_kwargs):
        ...
        return compute_cetian_ziwei_chart(**params_payload, **extra_kwargs)
    ...
    def _render(result: Any, params: BirthChartParams, options: dict[str, Any]) -> None:
        ...
        render_cetian_ziwei_chart(
            ...
            after_chart_hook=lambda: ai_button_sink(
                "tab_cetian", result, "cetian", ""
            ),
        )

    return SystemHandler(
        system_id="tab_cetian",
        compute=_compute,
        render=_render,
