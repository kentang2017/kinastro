"""開元占經 (Kaiyuan) Astrology handler."""
...
def build_kaiyuan_handler(
    *,
    compute_kaiyuan_chart,
    render_kaiyuan_chart,
    ai_button_sink,
) -> SystemHandler:
    """Create executable handler for 開元占經 (Kaiyuan) Astrology."""
    ...
    def _cached_compute(params_payload: dict[str, Any], **extra_kwargs):
        ...
        return compute_kaiyuan_chart(**params_payload, **extra_kwargs)
    ...
    def _render(result: Any, params: BirthChartParams, options: dict[str, Any]) -> None:
        ...
        render_kaiyuan_chart(
            ...
            after_chart_hook=lambda: ai_button_sink(
                "tab_kaiyuan", result, "kaiyuan", ""
            ),
        )

    return SystemHandler(
        system_id="tab_kaiyuan",
        compute=_compute,
        render=_render,
