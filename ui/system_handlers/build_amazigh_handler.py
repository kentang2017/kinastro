...
def build_amazigh_handler(
    *,
    compute_amazigh_chart,
    render_amazigh_chart,
    ai_button_sink,
) -> SystemHandler:
    """Create executable handler for 柏柏爾 (Amazigh) Astrology."""
    ...
    def _cached_compute(params_payload: dict[str, Any], **extra_kwargs):
        ...
        return compute_amazigh_chart(**params_payload, **extra_kwargs)
    ...
    def _render(result: Any, params: BirthChartParams, options: dict[str, Any]) -> None:
        ...
        render_amazigh_chart(
            ...
            after_chart_hook=lambda: ai_button_sink(
                "tab_amazigh", result, "amazigh", ""
            ),
        )

    return SystemHandler(
        system_id="tab_amazigh",
        compute=_compute,
        render=_render,
