"""凱爾特樹木 (Celtic Tree) Astrology handler."""
...
def build_celtic_handler(
    *,
    compute_celtic_tree_chart,
    render_celtic_tree_chart,
    ai_button_sink,
) -> SystemHandler:
    """Create executable handler for 凱爾特樹木 (Celtic Tree) Astrology."""
    ...
    def _cached_compute(params_payload: dict[str, Any], **extra_kwargs):
        ...
        return compute_celtic_tree_chart(**params_payload, **extra_kwargs)
    ...
    def _render(result: Any, params: BirthChartParams, options: dict[str, Any]) -> None:
        ...
        render_celtic_tree_chart(
            ...
            after_chart_hook=lambda: ai_button_sink(
                "tab_celtic", result, "celtic", ""
            ),
        )

    return SystemHandler(
        system_id="tab_celtic",
        compute=_compute,
        render=_render,
