"""波斯占星 (Persian) Astrology handler."""
...
def build_persian_handler(
    *,
    compute_deep_sassanian_chart,
    render_deep_sassanian_chart,
    ai_button_sink,
) -> SystemHandler:
    """Create executable handler for 波斯占星 (Persian) Astrology."""
    ...
    def _cached_compute(params_payload: dict[str, Any], **extra_kwargs):
        ...
        return render_deep_sassanian_chart(**params_payload, **extra_kwargs)
    ...
    def _render(result: Any, params: BirthChartParams, options: dict[str, Any]) -> None:
        ...
        render_deep_sassanian_chart(result)
        ai_button_sink("tab_persian", result, "persian", "")

    return SystemHandler(
        system_id="tab_persian",
        compute=_compute,
        render=_render,
