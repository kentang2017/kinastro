"""紅皮書 (Lal Kitab) Astrology handler."""
...
def build_lal_kitab_handler(
    *,
    compute_lal_kitab_chart,
    render_lal_kitab_chart,
    ai_button_sink,
) -> SystemHandler:
    """Create executable handler for 紅皮書 (Lal Kitab) Astrology."""
    ...
    def _cached_compute(params_payload: dict[str, Any], **extra_kwargs):
        ...
        return compute_lal_kitab_chart(**params_payload, **extra_kwargs)
    ...
    def _render(result: Any, params: BirthChartParams, options: dict[str, Any]) -> None:
        ...
        render_lal_kitab_chart(
            ...
            after_chart_hook=lambda: ai_button_sink(
                "tab_lal_kitab", result, "lal_kitab", ""
            ),
        )

    return SystemHandler(
        system_id="tab_lal_kitab",
        compute=_compute,
        render=_render,
