"""滌器遺訣 (Diqiyijue) Astrology handler."""
...
def build_diqiyijue_handler(
    *,
    compute_diqiyijue_chart,
    render_diqiyijue_chart,
    ai_button_sink,
) -> SystemHandler:
    """Create executable handler for 滌器遺訣 (Diqiyijue) Astrology."""
    ...
    def _cached_compute(params_payload: dict[str, Any], **extra_kwargs):
        ...
        return compute_diqiyijue_chart(**params_payload, **extra_kwargs)
    ...
    def _render(result: Any, params: BirthChartParams, options: dict[str, Any]) -> None:
        ...
        render_diqiyijue_chart(
            ...
            after_chart_hook=lambda: ai_button_sink(
                "tab_diqiyijue", result, "diqiyijue", ""
            ),
        )

    return SystemHandler(
        system_id="tab_diqiyijue",
        compute=_compute,
        render=_render,
