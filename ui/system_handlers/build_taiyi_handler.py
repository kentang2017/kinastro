...
def build_taiyi_handler(
    *,
    compute_taiyi_chart,
    render_taiyi_chart,
    ai_button_sink,
) -> SystemHandler:
    """Create executable handler for 太乙 (Taiyi) Astrology."""
...
def register(registry, ai_button_sink):
    """Self-registration for 太乙 (Taiyi) handler (modular lazy pattern)."""
    from astro.sanshi import compute_taiyi_chart, render_taiyi_chart

    handler = build_taiyi_handler(
        compute_taiyi_chart=compute_taiyi_chart,
        render_taiyi_chart=render_taiyi_chart,
