"""十二星次 (Twelve Ci) Astrology handler."""
...
def build_twelve_ci_handler(
    *,
    compute_twelve_ci_chart,
    render_twelve_ci_chart,
    ai_button_sink,
) -> SystemHandler:
    """Create executable handler for 十二星次 (Twelve Ci) Astrology."""
    ...
    def _cached_compute(params_payload: dict[str, Any], **extra_kwargs):
        ...
        return compute_twelve_ci_chart(**params_payload, **extra_kwargs)
    ...
    def _render(result: Any, params: BirthChartParams, options: dict[str, Any]) -> None:
        ...
        render_twelve_ci_chart(
            ...
            after_chart_hook=lambda: ai_button_sink(
                "tab_twelve_ci", result, "twelve_ci", ""
            ),
        )

    return SystemHandler(
        system_id="tab_twelve_ci",
        compute=_compute,
        render=_render,
...
def register(registry, ai_button_sink):
    """Self-registration for Twelve Ci handler."""
    from astro.twelve_ci import compute_twelve_ci_chart, render_twelve_ci_chart, build_twelve_ci_svg

    handler = build_twelve_ci_handler(
        compute_twelve_ci_chart=compute_twelve_ci_chart,
        render_twelve_ci_chart=render_twelve_ci_chart,
