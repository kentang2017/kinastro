"""九星氣學 (Nine Star Ki) Astrology handler."""
...
def build_nine_star_ki_handler(
    *,
    compute_nine_star_ki_chart,
    render_nine_star_ki_chart,
    ai_button_sink,
) -> SystemHandler:
    """Create executable handler for 九星氣學 (Nine Star Ki) Astrology."""
    ...
    def _cached_compute(params_payload: dict[str, Any], **extra_kwargs):
        ...
        return compute_nine_star_ki_chart(**params_payload, **extra_kwargs)
    ...
    def _render(result: Any, params: BirthChartParams, options: dict[str, Any]) -> None:
        ...
        render_nine_star_ki_chart(
            ...
            after_chart_hook=lambda: ai_button_sink(
                "tab_nine_star_ki", result, "nine_star_ki", ""
            ),
        )

    return SystemHandler(
        system_id="tab_nine_star_ki",
        compute=_compute,
        render=_render,
...
def register(registry, ai_button_sink):
    """Self-registration for 九星紀 (Nine Star Ki) handler (modular lazy pattern)."""
    from astro.nine_star_ki import compute_nine_star_ki_chart, render_nine_star_ki_chart

    handler = build_nine_star_ki_handler(
        compute_nine_star_ki_chart=compute_nine_star_ki_chart,
        render_nine_star_ki_chart=render_nine_star_ki_chart,
