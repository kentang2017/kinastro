"""蒙古祖爾海 (Zurkhai) Astrology handler."""
...
def build_zurkhai_handler(
    *,
    compute_zurkhai_chart,
    render_zurkhai_chart,
    ai_button_sink,
) -> SystemHandler:
    """Create executable handler for 蒙古祖爾海 (Zurkhai) Astrology."""
    ...
    def _cached_compute(params_payload: dict[str, Any], **extra_kwargs):
        ...
        return compute_zurkhai_chart(**params_payload, **extra_kwargs)
...
def register(registry, ai_button_sink):
    """Self-registration for 蒙古祖爾海 (Zurkhai) handler (modular lazy pattern)."""
    from astro.zurkhai import compute_zurkhai_chart, render_zurkhai_chart

    handler = build_zurkhai_handler(
        compute_zurkhai_chart=compute_zurkhai_chart,
        render_zurkhai_chart=render_zurkhai_chart,
