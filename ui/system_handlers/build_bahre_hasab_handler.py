"""衣索比亞 (Bahre Hasab) Astrology handler."""
...
def build_bahre_hasab_handler(
    *,
    compute_bahre_hasab,
    render_bahre_hasab,
    ai_button_sink,
) -> SystemHandler:
    """Create executable handler for 衣索比亞 (Bahre Hasab) Astrology."""
    ...
    def _cached_compute(params_payload: dict[str, Any], **extra_kwargs):
        ...
        return analyze_bahre_hasab_date(**params_payload, **extra_kwargs)
    ...
    def _render(result: Any, params: BirthChartParams, options: dict[str, Any]) -> None:
        ...
        render_bahre_hasab(
            ...
            after_chart_hook=lambda: ai_button_sink(
                "tab_bahre_hasab", result, "bahre_hasab", ""
            ),
        )

    return SystemHandler(
        system_id="tab_bahre_hasab",
        compute=_compute,
        render=_render,
