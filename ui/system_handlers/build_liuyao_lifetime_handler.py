"""六爻終身卦 (Lifetime Liu Yao) Astrology handler."""
...
def build_liuyao_lifetime_handler(
    *,
    compute_lifetime_hexagram,
    render_liuyao_lifetime_chart,
    ai_button_sink,
) -> SystemHandler:
    """Create executable handler for 六爻終身卦 (Lifetime Liu Yao) Astrology."""
    ...
    def _cached_compute(params_payload: dict[str, Any], **extra_kwargs):
        ...
        return compute_lifetime_hexagram(**params_payload, **extra_kwargs)
    ...
    def _render(result: Any, params: BirthChartParams, options: dict[str, Any]) -> None:
        ...
        render_liuyao_lifetime_chart(
            ...
            after_chart_hook=lambda: ai_button_sink(
                "tab_liuyao_lifetime", result, "liuyao_lifetime", ""
            ),
        )

    return SystemHandler(
        system_id="tab_liuyao_lifetime",
        compute=_compute,
        render=_render,
