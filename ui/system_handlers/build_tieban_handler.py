...
def build_tieban_handler(
    *,
    ...
    def _cached_compute(params_payload: dict[str, Any], **extra_kwargs):
        ...
        return render_tieban_chart_svg(**params_payload, **extra_kwargs)
    ...
    def _render(result: Any, params: BirthChartParams, options: dict[str, Any]) -> None:
        ...
        # render_tieban_chart_svg does not accept after_chart_hook
        render_tieban_chart_svg(result)
...
def register(registry, ai_button_sink):
    ...
    handler = build_tieban_handler(
        compute_tieban_chart=lambda **kw: TieBanShenShu().compute(**kw),
