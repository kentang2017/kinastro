"""Picatrix (Picatrix) Astrology handler."""
...
def build_picatrix_handler(
    *,
    compute_picatrix,
    render_picatrix,
    ai_button_sink,
) -> SystemHandler:
    """Create executable handler for Picatrix (Picatrix) Astrology."""
    ...
    def _cached_compute(params_payload: dict[str, Any], **extra_kwargs):
        ...
        return render_picatrix_behenian(**params_payload, **extra_kwargs)
    ...
    def _render(result: Any, params: BirthChartParams, options: dict[str, Any]) -> None:
        ...
        render_picatrix(result)
        ai_button_sink("tab_picatrix", result, "picatrix", "")

    return SystemHandler(
        system_id="tab_picatrix",
        compute=_compute,
        render=_render,
