"""猶太 Mazzalot (Mazzalot) Astrology handler."""
...
def build_mazzalot_handler(
    *,
    compute_mazzalot_chart,
    render_mazzalot_chart,
    ai_button_sink,
) -> SystemHandler:
    """Create executable handler for 猶太 Mazzalot (Mazzalot) Astrology."""
    ...
    def _cached_compute(params_payload: dict[str, Any], **extra_kwargs):
        ...
        return compute_mazzalot_chart(**params_payload, **extra_kwargs)
    ...
    def _render(result: Any, params: BirthChartParams, options: dict[str, Any]) -> None:
        ...
        render_mazzalot_chart(
            ...
            after_chart_hook=lambda: ai_button_sink(
                "tab_mazzalot", result, "mazzalot", ""
            ),
        )

    return SystemHandler(
        system_id="tab_mazzalot",
        compute=_compute,
        render=_render,
