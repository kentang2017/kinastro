...
def build_yemeni_handler(
    *,
    compute_yemeni_chart,
    render_yemeni_chart,
    ai_button_sink,
) -> SystemHandler:
    """Create executable handler for 也門占星 (Yemeni) Astrology."""
    ...
    def _cached_compute(params_payload: dict[str, Any], **extra_kwargs):
        ...
        return compute_yemeni_chart(**params_payload, **extra_kwargs)
    ...
    def _render(result: Any, params: BirthChartParams, options: dict[str, Any]) -> None:
        ...
        render_yemeni_chart(
            ...
            after_chart_hook=lambda: ai_button_sink(
                "tab_yemeni", result, "yemeni", ""
            ),
        )

    return SystemHandler(
        system_id="tab_yemeni",
        compute=_compute,
        render=_render,
