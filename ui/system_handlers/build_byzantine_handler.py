"""拜占庭 (Byzantine) Astrology handler."""
...
def build_byzantine_handler(
    *,
    compute_byzantine_chart,
    render_byzantine_chart,
    ai_button_sink,
) -> SystemHandler:
    """Create executable handler for 拜占庭 (Byzantine) Astrology."""
    ...
    def _cached_compute(params_payload: dict[str, Any], **extra_kwargs):
        ...
        return compute_byzantine_chart(**params_payload, **extra_kwargs)
    ...
    def _render(result: Any, params: BirthChartParams, options: dict[str, Any]) -> None:
        ...
        render_byzantine_chart(
            ...
            after_chart_hook=lambda: ai_button_sink(
                "tab_byzantine", result, "byzantine", ""
            ),
        )

    return SystemHandler(
        system_id="tab_byzantine",
        compute=_compute,
        render=_render,
