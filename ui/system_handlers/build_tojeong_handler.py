...
def build_tojeong_handler(
    *,
    compute_tojeong_chart,
    render_tojeong_chart,
    ai_button_sink,
) -> SystemHandler:
    """Create executable handler for 土亭數 (Tojeong) Astrology."""
    ...
    def _cached_compute(params_payload: dict[str, Any], **extra_kwargs):
        ...
        return compute_tojeong_chart(**params_payload, **extra_kwargs)
    ...
    def _render(result: Any, params: BirthChartParams, options: dict[str, Any]) -> None:
        ...
        render_tojeong_chart(
            ...
            after_chart_hook=lambda: ai_button_sink(
                "tab_tojeong", result, "tojeong", ""
            ),
        )

    return SystemHandler(
        system_id="tab_tojeong",
        compute=_compute,
        render=_render,
