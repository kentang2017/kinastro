"""太玄數 (Tai Xuan) Astrology handler."""
...
def build_taixuan_handler(
    *,
    compute_taixuan_chart,
    render_taixuan_chart,
    ai_button_sink,
) -> SystemHandler:
    """Create executable handler for 太玄數 (Tai Xuan) Astrology."""
    ...
    def _cached_compute(params_payload: dict[str, Any], **extra_kwargs):
        ...
        return render_taixuan_chart(**params_payload, **extra_kwargs)
    ...
    def _render(result: Any, params: BirthChartParams, options: dict[str, Any]) -> None:
        ...
        render_taixuan_chart(
            ...
            after_chart_hook=lambda: ai_button_sink(
                "tab_taixuan", result, "taixuan", ""
            ),
        )

    return SystemHandler(
        system_id="tab_taixuan",
        compute=_compute,
        render=_render,
