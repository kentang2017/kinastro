...
def build_qimen_handler(
    *,
    compute_qimen_chart,
    render_qimen_chart,
    ai_button_sink,
) -> SystemHandler:
    """Create executable handler for 奇門 (Qimen) Astrology."""
    ...
    def _cached_compute(params_payload: dict[str, Any], **extra_kwargs):
        ...
        return compute_taiyi_chart(**params_payload, **extra_kwargs)
...
def register(registry, ai_button_sink):
    """Self-registration for 奇門 (Qimen) handler (modular lazy pattern)."""
    from astro.sanshi import compute_qimen_chart, render_qimen_chart

    handler = build_qimen_handler(
        compute_qimen_chart=compute_qimen_chart,
        render_qimen_chart=render_qimen_chart,
