...
def build_geomancy_handler(
    *,
    compute_geomancy_chart,
    render_geomancy_chart,
    ai_button_sink,
) -> SystemHandler:
    """Create executable handler for 地占 (Geomancy) Astrology."""
    ...
    def _cached_compute(params_payload: dict[str, Any], **extra_kwargs):
        ...
        return compute_geomancy_chart(**params_payload, **extra_kwargs)
    ...
    def _render(result: Any, params: BirthChartParams, options: dict[str, Any]) -> None:
        ...
        render_geomancy_chart(
            ...
            after_chart_hook=lambda: ai_button_sink(
                "tab_geomancy", result, "geomancy", ""
            ),
        )

    return SystemHandler(
        system_id="tab_geomancy",
        compute=_compute,
        render=_render,
