"""宿曜道 (Sukkayodo) Astrology handler."""
...
def build_sukkayodo_handler(
    *,
    compute_sukkayodo_chart,
    render_sukkayodo_chart,
    ai_button_sink,
) -> SystemHandler:
    """Create executable handler for 宿曜道 (Sukkayodo) Astrology."""
    ...
    def _cached_compute(params_payload: dict[str, Any], **extra_kwargs):
        ...
        return render_sukkayodo_chart(**params_payload, **extra_kwargs)
    ...
    def _render(result: Any, params: BirthChartParams, options: dict[str, Any]) -> None:
        ...
        render_sukkayodo_chart(
            ...
            after_chart_hook=lambda: ai_button_sink(
                "tab_sukkayodo", result, "sukkayodo", ""
            ),
        )

    return SystemHandler(
        system_id="tab_sukkayodo",
        compute=_compute,
        render=_render,
