"""伊特魯里亞 (Etruscan) Astrology handler."""
...
def build_etruscan_handler(
    *,
    compute_etruscan_chart,
    render_etruscan_chart_ui,
    ai_button_sink,
) -> SystemHandler:
    """Create executable handler for 伊特魯里亞 (Etruscan) Astrology."""
    ...
    def _cached_compute(params_payload: dict[str, Any], **extra_kwargs):
        ...
        return compute_etruscan_chart(**params_payload, **extra_kwargs)
    ...
    def _render(result: Any, params: BirthChartParams, options: dict[str, Any]) -> None:
        ...
        render_etruscan_chart_ui(
            ...
            after_chart_hook=lambda: ai_button_sink(
                "tab_etruscan", result, "etruscan", ""
            ),
        )

    return SystemHandler(
        system_id="tab_etruscan",
        compute=_compute,
        render=_render,
