...
def build_beiji_handler(
    *,
    ...
    """Create executable handler for 北極神數 (Beiji) Astrology."""
    ...
    def _render(result: Any, params: BirthChartParams, options: dict[str, Any]) -> None:
        ...
        # render_beiji_chart does not accept after_chart_hook
        render_beiji_chart(result)
        ai_button_sink("tab_beiji", result, "beiji", "")
...
def register(registry, ai_button_sink):
    """Self-registration for Beiji handler."""
    from astro.beiji import compute_beiji as compute_beiji_chart, render_beiji_chart

    handler = build_beiji_handler(
        compute_beiji_chart=compute_beiji_chart,
        render_beiji_chart=render_beiji_chart,
