"""藏傳占星 (Tibetan) Astrology handler."""
...
def build_tibetan_handler(
    *,
    compute_tibetan_chart,
    render_tibetan_chart,
    ai_button_sink,
) -> SystemHandler:
    """Create executable handler for 藏傳占星 (Tibetan) Astrology."""
    ...
    def _cached_compute(params_payload: dict[str, Any], **extra_kwargs):
        ...
        return compute_tibetan_chart(**params_payload, **extra_kwargs)
    ...
    def _render(result: Any, params: BirthChartParams, options: dict[str, Any]) -> None:
        ...
        render_tibetan_chart(
            ...
            after_chart_hook=lambda: ai_button_sink(
                "tab_tibetan", result, "tibetan", ""
            ),
        )

    return SystemHandler(
        system_id="tab_tibetan",
        compute=_compute,
        render=_render,
...
def register(registry, ai_button_sink):
    """Self-registration for 藏傳占星 (Tibetan) handler (modular lazy pattern)."""
    from astro.tibetan import compute_tibetan_chart, render_tibetan_chart

    handler = build_tibetan_handler(
        compute_tibetan_chart=compute_tibetan_chart,
        render_tibetan_chart=render_tibetan_chart,
