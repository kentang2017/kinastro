"""Sabian 符號 (Sabian Symbols) Astrology handler."""
...
def build_sabian_handler(
    *,
    compute_sabian_chart,
    render_sabian_chart,
    ai_button_sink,
) -> SystemHandler:
    """Create executable handler for Sabian 符號 (Sabian Symbols) Astrology."""
    ...
    def _cached_compute(params_payload: dict[str, Any], **extra_kwargs):
        ...
        return compute_western_chart(**params_payload, **extra_kwargs)
    ...
    def _render(result: Any, params: BirthChartParams, options: dict[str, Any]) -> None:
        ...
        render_sabian_chart(
            ...
            after_chart_hook=lambda: ai_button_sink(
                "tab_sabian", result, "sabian", ""
            ),
        )

    return SystemHandler(
        system_id="tab_sabian",
        compute=_compute,
        render=_render,
