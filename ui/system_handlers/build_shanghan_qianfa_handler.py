"""傷寒鈐法 (Shanghan Qianfa) Astrology handler."""
...
def build_shanghan_qianfa_handler(
    *,
    compute_shanghan_qianfa,
    render_shanghan_qianfa_chart,
    ai_button_sink,
) -> SystemHandler:
    """Create executable handler for 傷寒鈐法 (Shanghan Qianfa) Astrology."""
    ...
    def _cached_compute(params_payload: dict[str, Any], **extra_kwargs):
        ...
        return compute_shanghan_qianfa(**params_payload, **extra_kwargs)
    ...
    def _render(result: Any, params: BirthChartParams, options: dict[str, Any]) -> None:
        ...
        render_shanghan_qianfa_chart(
            ...
            after_chart_hook=lambda: ai_button_sink(
                "tab_shanghan_qianfa", result, "shanghan_qianfa", ""
            ),
        )

    return SystemHandler(
        system_id="tab_shanghan_qianfa",
        compute=_compute,
        render=_render,
