...
def build_western_wheel_svg(chart: WesternChart, gender: Optional[str] = None) -> str:
        ...
        for c, idx in enumerate(row_data):
            ...
            if idx == -1:
                ...
                parts.append(
                    ...
                    f'fill="#aaa" font-size="11">{_xml_escape(tz_str)}</text>'
                )
                ...
                if gender:
                    ...
                    parts.append(
                        f'<text x="{mcx}" y="{mcy + 10}" text-anchor="middle" '
                        f'fill="#e0e0e0" font-size="12">{_xml_escape(gender_label)}</text>'
                    )
                ...
                if chart.location_name:
                    parts.append(
                        ...
                        f'fill="#aaa" font-size="11">{_xml_escape(chart.location_name)}</text>'
                    )
                continue
            ...
            parts.append(
                ...
                f'fill="#aaa" font-size="10">{_xml_escape(_format_deg(h.cusp))}</text>'
            )
            ...
            if planets_in_house:
                ...
                for i, (short, full) in enumerate(
                    ...
                    parts.append(
                        ...
                        f'font-weight="bold">{_xml_escape(short)}</text>'
                    )
            else:
                parts.append(
...
def _render_wheel_chart(chart: WesternChart, gender: Optional[str] = None) -> None:
    ...
    st.markdown(build_western_wheel_svg(chart, gender=gender), unsafe_allow_html=True)
