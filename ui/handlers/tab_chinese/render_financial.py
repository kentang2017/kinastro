...
def render_stock_fortune_tab(input_tz: float = 8.0):
    ...
    from astro.qizheng.financial.stock_fetcher import fetch_stock_info, get_display_name, get_price_ratio, get_strength_label
    from astro.qizheng.financial.stock_calculator import compute_stock_chart, compute_daily_fortune
...
def _render_stock_card(stock):
    ...
    from astro.qizheng.financial.stock_fetcher import get_display_name
...
def _render_daily_fortune(stock_data, go, query_date: date, query_hour: int):
    """渲染流日流時吉凶分頁"""
    from astro.qizheng.financial.stock_calculator import DailyFortuneData
...
def _render_hourly_timeline(go, query_date: date, timezone: float):
    """渲染今日 24 小時各時辰吉凶折線圖"""
    from astro.qizheng.financial.stock_calculator import compute_daily_fortune
...
def _build_gann_stock_context(ipo_date: date, query_date: date, query_hour: int, timezone: float) -> Optional[dict]:
    ...
    from astro.qizheng.financial.gann_macro_stock import build_gann_macro_timing
...
def _wuxing_bar(dist: dict, total: int, title: str):
    ...
    import streamlit as st
    from astro.qizheng.financial.name_wuxing import WUXING_ELEMENTS, WUXING_COLORS
...
def _compatibility_grade(
    ...
    *,
    ...
    from astro.qizheng.financial.name_wuxing import WUXING_KE, WUXING_KEME, WUXING_SHENG, WUXING_SHENGME
...
def _merge_wuxing_distributions(*distributions: dict[str, int]) -> dict[str, int]:
    """合併多個五行分佈，用於綜合評級。"""
    from astro.qizheng.financial.name_wuxing import WUXING_ELEMENTS
...
def _get_day_master_profile(bazi_result: dict) -> tuple[str, str]:
    """回傳日主五行與簡化身強弱描述。"""
    from astro.qizheng.financial.name_wuxing import WUXING_KE, WUXING_KEME, WUXING_SHENGME
...
def _render_name_wuxing(stock):
    ...
    """
    from astro.qizheng.financial.name_wuxing import (
        analyze_name_wuxing,
